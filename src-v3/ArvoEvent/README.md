...
| `parentid`        | ArvoEvent Extension | The unique identifier of the event that directly triggered the creation of this event within the Arvo ecosystem. Establishes direct causal relationships for event lineage tracking and workflow orchestration. Essential for debugging event chains and understanding process flows.                                                             |
| `domain`          | ArvoEvent Extension | Specifies the processing domain for event routing and workflow orchestration. Enables specialized processing flows such as human-in-the-loop operations, third-party integrations, or custom handling pipelines. Each event can only belong to one domain, supporting targeted processing while maintaining clear boundaries between different operational contexts. |
| `traceparent`     | OpenTelemetry Extension | Part of OpenTelemetry integration. Carries distributed tracing context across service boundaries. Essential for understanding event flow and diagnosing issues in complex distributed systems.                                                                                                                                                    |
| `tracestate`      | OpenTelemetry Extension | Supports vendor-specific tracing information as part of OpenTelemetry integration. Enables detailed tracing and monitoring capabilities while maintaining compatibility with various monitoring tools.                                                                                                                                            |
