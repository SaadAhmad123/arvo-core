...
- [x] 1.1 Probe that `createOutput`'s output key narrows against the real classes, not a hand-written shape: `type` constrained to `keyof V['output'] & string` rejects an undeclared key at the call site, and the payload type follows the key named. The sketch compiles, but against one contract — repeat it for a version declaring several emits and for one declaring none.
- [x] 1.2 Probe that `z.output<V['input']>` differs from `z.input<V['input']>` where the schema declares a default, so the two-sided typing in `types.ts` is load-bearing rather than decorative.
...
- [x] 4.4 Confirm every variant reads its derived values off the contract — `contract.type`, `contract.dataschema`, `contract.error.type`, `contract.error.schema` — and derives none of them a second time.
...
- [x] 6.8 Add `tests/factories/agreement.spec.ts` — the property that ties this change to its sibling: an event built by `createInput`, `createOutput` and `createError` in turn, asserted back against the version that built it, matches with the scope that builder implies.
...
- [x] 8.3 Write the TSDoc per `project.md` for everything newly exported: rules, not provenance, and each member standing alone rather than pointing at its twin. State on each builder that the payload comes back as the declaration produced it, so a caller comparing what they passed is not surprised; on `createInput` that the recipient defaults to the contract's type; on `createOutput` and `createError` that it does not; and on `cloneArvoEvent` that identity and time are copied, so a clone sent alongside its source needs a new `id`.
- [x] 8.4 Not done, and not doable as written: `tsconfig.json` excludes `tests`, so no command in this package type-checks a test file. A type-level spec would be verified by an editor and by nothing else. The narrowing §1 established is exercised instead by the runtime specs, which fail if a payload's shape changes.
- [x] 8.5 Add a section to `ts/sandbox/src/playground.ts` exercising the factory's three builders, the two standalone pairs, each of the four domain sources, and the three failures a caller is likeliest to meet.
...
- [x] 9.1 Run `pnpm test --coverage`, `pnpm lint` and `npx tsc --noEmit`; hold `src/factories/` at the 100% line, branch and function coverage the package holds. The `createOutput` guard, the non-`Error` path and the factory's contract guard are reachable only from untyped callers — cover them, do not delete them.
...
- [x] 9.3 Confirm no field is filled in with a value nothing knows. `subject` is generated, and `to` on `createInput` comes from the contract; every other absent field is left absent.
