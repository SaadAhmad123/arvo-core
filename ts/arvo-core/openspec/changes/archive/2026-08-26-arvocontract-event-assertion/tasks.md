...
- [x] 1.1 Probe the narrowing path's conditional types against the real generic shape, not a mock: the contract's `type`, an output key, and the handler error type each narrow the reported scope to a single literal, the payload type follows the expected type, and an undeclared type is rejected at the call site. Recorded in design.md as already probed on a mock — repeat it against the actual classes, since a conditional that resolves on a hand-written shape can still collapse when the output keys arrive through a generic.
...
- [x] 4.4 Add `tests/ArvoContract/assert-version.spec.ts`: each of the three shapes matching, the handler error assertable when `outputs` is empty, an event matching none, a correct expectation, an expectation the event contradicts, an expectation the version does not declare, and no expectation at all. The contradicted expectation reports at `event.type`; the undeclarable one at `expectedType`.
