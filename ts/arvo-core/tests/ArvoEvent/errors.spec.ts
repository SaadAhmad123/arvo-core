...
const messageFor = (received: unknown): string =>
  new ArvoEventValidationError([
    new ErrorIssue({ path: 'field', message: 'is wrong', received }),
  ]).message;
...
describe('ArvoEventValidationError', () => {
  describe('issue rendering', () => {
    ...
    it('omits the received clause when an issue carries no received value', () => {
      const error = new ArvoEventValidationError([
        new ErrorIssue({ path: 'data.fn', message: 'is a function' }),
      ]);
      expect(error.message).not.toContain('received');
    });

    it('omits the received clause when received is explicitly undefined', () => {
      // Absent and explicitly undefined are indistinguishable once an issue
      // is constructed, so both read as "no value to show" rather than as
      // "the value was undefined". A rule about undefined says so in its
      // message instead.
      const error = new ArvoEventValidationError([
        new ErrorIssue({
          path: 'depth',
    ...
    it('renders a combined cross-field path unchanged', () => {
      const error = new ArvoEventValidationError([
        new ErrorIssue({ path: 'parentid + depth', message: 'must be 0' }),
      ]);
  ...
  describe('received value rendering, one case per type', () => {
    ...
    it('shows no clause at all for undefined, rather than rendering it', () => {
      // The one value describeValue cannot be reached with from here: an
      // issue omits the clause entirely when received is undefined. Its
      // 'undefined' rendering is covered through a stage cause in
      // tests/cloudevent/errors.spec.ts.
      expect(messageFor(undefined)).not.toContain('received');
    });
  ...
  describe('message shape', () => {
    it('reads as one sentence for a single issue', () => {
      ...
      expect(error.message).toBe(
        [
          'ArvoEvent is not structurally valid.',
    ...
    it('reads as a counted list for several issues', () => {
      const error = new ArvoEventValidationError([
        new ErrorIssue({ path: 'subject', message: 'is required' }),
        new ErrorIssue({ path: 'source', message: 'is required' }),
        new ErrorIssue({ path: 'type', message: 'is required' }),
      ]);
      expect(error.message).toBe(
        [
          'ArvoEvent is not structurally valid.',
          'The following 3 problems were found:',
          '  - subject: is required',
          '  - source: is required',
          '  - type: is required',
        ].join('\n'),
  ...
  describe('the error object itself', () => {
    ...
    it('freezes issues so a caller cannot mutate the record of what failed', () => {
      ...
      expect(() => {
        (error.issues as ErrorIssue[]).push(
          new ErrorIssue({ path: 'x', message: 'y' }),
        );
      }).toThrow();
    });
    ...
    it('copies the issues it was given, so mutating the input afterwards does not change it', () => {
      ...
      const error = new ArvoEventValidationError(issues);
      issues.push(new ErrorIssue({ path: 'source', message: 'is required' }));
