...
/**
 * Building events from a contract.
 *
 * The factory holds one version, so it supplies each event's type, dataschema
 * and payload rules and the caller never repeats them.
 */
function buildingEventsFromAContract(): void {
	section("Building events from a contract");
	...
	console.log(`  accepted: ${requested.type} -> ${requested.to}`);
	...
	console.log(`  emitted:  ${emitted.type} -> ${emitted.to}`);
	...
	// Domain: absent, literal, or read from a named source.
	...
	const domains: Array<[string, string | null]> = [
		...
		[
			...
			factory.createInput(
				...
				{
					domainCtx: {
						// An event carrying a domain of its own, so the symbol has
						// something to read.
						triggeringEvent: createArvoEvent({
							type: "com_order_create",
							source: "com.inbound",
	...
	for (const [named, resolved] of domains) {
		console.log(`    ${named.padEnd(36)} ${resolved ?? "(none)"}`);
	}

	// No contract to read from, and a copy of an existing event.
	const standalone = createArvoEvent({
	...
	console.log(
		`\n  standalone: ${standalone.type}, subject ${standalone.subject.slice(0, 8)}…`,
	);
	console.log(
		`  clone:      to ${routed.to}, same id ${routed.id === emitted.id}`,
	);

	// The three failures a caller is likeliest to meet, each reported.
	console.log("\n  what a caller gets wrong, and where it is reported:");
	const failures: Array<[string, () => unknown]> = [
		[
			"a payload the schema rejects",
			() =>
