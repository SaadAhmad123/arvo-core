...
import { createArvoEvent, cloneArvoEvent, tryCloneArvoEvent } from "arvo-core";
...
const standalone = (): void => {
	...
	console.log("  subject:", `${event.subject.slice(0, 8)}… (defaulted to a UUID)`);
};
...
const traceContextPrecedence = (): void => {
	...
	const show = (label: string, event: { traceparent: string | null; tracestate: string | null }) =>
		console.log(
