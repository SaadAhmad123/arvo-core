...
The same rule governs the inbound direction. A canonical form may contain a construct a given language's validation library cannot model, and that implementation MUST omit it rather than approximate it: the contract it constructs is a true, if weaker, materialization of the form it was built from. It MUST NOT substitute a check the canonical form did not specify, and it MUST NOT reject a canonical form solely because part of it was inexpressible — a contract held weakly is still that contract, and refusing to hold it at all would put the least capable library in every ecosystem in charge of what the model can express. An implementation SHOULD make such omissions discoverable rather than silent, so that an operator can see where its own materialization is weaker than the form it was built from; how it does so is its own choice.
