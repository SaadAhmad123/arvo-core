...
Each key of `versions` MUST be a valid semantic version in `MAJOR.MINOR.PATCH` form. `versions` MUST contain at least one entry — a contract declaring none has no schema for any handler to implement and nothing for `uri` or `type` to attach to, so it is not well formed.
