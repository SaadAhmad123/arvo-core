...
This is the form every language implementation MUST be able to produce from its own natively-authored contract, and MUST be able to construct a working contract object from, given a canonical form produced elsewhere — including one authored in a different language. Neither direction promises that nothing is lost in the crossing; see the best-effort rule below. A language's own idiomatic validation object — whatever native schema representation that language's own ecosystem favors — is a materialization of the canonical form for that language's own ergonomics. It is not the source of truth; the canonical JSON form is.

**The canonical form is a floor, not a ceiling.** It is the guaranteed common ground — what every implementation can produce, and what every implementation can consume, whatever language authored it. That is the sense in which it is the source of truth: whether two participants written in different languages hold the same contract is answered here and nowhere else.

It is not a statement of everything a given node will reject. A native validation object may enforce more than the canonical form can express (see the best-effort rule below), and that additional strictness is local to the implementation that authored it — expected, permitted, and no part of the contract's identity. A reader of a canonical form learns what every implementation of that contract agrees to enforce, not the most that any one of them does.

**Conversion is best-effort in both directions, not a semantic-equivalence guarantee.** A native validation object may express a constraint JSON Schema 2020-12 cannot — a custom predicate, a transform, a refinement checked by arbitrary code — and this ADR does not restrict that authoring-time richness; it governs the canonical form, not what a language's own validation library is allowed to express while authoring one. When a native construct has no JSON Schema 2020-12 equivalent, the canonical export MUST omit it rather than approximate it. The exported schema becomes a true, if weaker, subset of what the native validator actually enforces — never a fabricated stand-in implying a check the schema doesn't actually perform. Two implementations MUST NOT invent different heuristic approximations for the same unrepresentable construct; omission is the only permitted response to inexpressibility.

The same rule governs the inbound direction. A canonical form may contain a construct a given language's validation library cannot model, and that implementation MUST omit it rather than approximate it: the contract it constructs is a true, if weaker, materialization of the form it was built from. It MUST NOT substitute a check the canonical form did not specify, and it MUST NOT reject a canonical form solely because part of it was inexpressible — a contract held weakly is still that contract, and refusing to hold it at all would put the least capable library in every ecosystem in charge of what the model can express. An implementation SHOULD make an omission in either direction discoverable rather than silent, so that an author can see where a canonical form is weaker than the contract they wrote, and an operator can see where a materialization is weaker than the form it was built from. This covers a check demoted as well as one dropped: a native check that JSON Schema can express only as an annotation survives in the form as documentation that nothing may enforce, which stops it working just as surely as omitting it would. How an implementation reports either is its own choice.
...
**Paid for.** Contract-declared identifiers give up the dotted, CloudEvents-familiar naming shown throughout existing documentation and examples — a real, visible departure from convention, even though CloudEvents itself never required it. Every language implementation now owns one more precise, cross-language-testable obligation: the `uri`-derivation algorithm must match exactly, not merely produce a "reasonable" result. And because an inexpressible construct is omitted rather than approximated, in either direction, validation is looser at the far end of every crossing: a contract exported from a language whose author used a construct JSON Schema cannot carry validates more loosely everywhere else, and a contract read into a library that cannot model part of the form it was given is held more loosely there. The canonical form only ever guarantees the subset of validation JSON Schema can express and every implementation can model, not whatever the richest one happens to enforce. A check a native library expresses only as an annotation is a quieter case of the same cost: it survives the crossing as documentation, is unenforceable by the rule above, and so stops being a check at all — including for the implementation that authored it, reading its own file back.
