...
A nullable field (`parentid`, `initid`, `category`, `to`, `domain`, `executionunits`) that is `null` on the ArvoEvent is omitted from the CloudEvent's extension attributes entirely — CloudEvents' extension type system has no `null` value, so absence is how null is represented. Absence on deserialization is read back as `null`, never as an error, consistent with ADR-001 already treating `null` and absent as equivalent for these fields.
...
| `traceparent` | extension `traceparent` |
| `tracestate` | extension `tracestate` |
...
- A **foreign CloudEvent** — one Arvo never produced, lacking some or all of the extension attributes above, and potentially lacking `subject` or `dataschema`, both required by ADR-001 but optional in CloudEvents core. This is not a degenerate case; ADR-001 already anticipates it directly: "events from foreign systems enter through a boundary that declares a contract on their behalf and supplies the value."
...
**`baggage` as a JSON-encoded string extension attribute** — considered, not chosen. It would have kept `data` byte-identical to `ArvoEvent.data`, preserving direct payload transparency for a generic CloudEvents consumer. Rejected because, once `data` already carries `arvoeventdata` as a wrapper key, nesting `baggage` there directly is simpler than a second encoding step, and losslessness holds either way.
...
**Invariants strained.** None, with one caveat recorded rather than resolved: placing `baggage` inside `data` means a CloudEvents-only consumer no longer sees the payload alone by reading `data.arvoeventdata` — that consumer needs Arvo's wrapper convention to reach it, which is a real, if narrow, departure from ADR-001's original goal that `data` be directly consumable by "a human task queue, a foreign system, or a service someone stands up in an afternoon." The **Considered Alternatives** section above records what was weighed against it.
