...
- **Resolves:** ADR-000 Deferred Decision — "Cross-language protocol compatibility"
...
Each language package maintains its own `openspec/` directory (`ts/arvo-core/openspec/`, `py/arvo-core/openspec/`, and so on), including its own `project.md` stating that language's own conventions. OpenSpec instantiates an ADR's requirements into concrete, language-specific implementation decisions — types, error shapes, verified dependency behavior, tests. This detail does not transfer between languages and is not centralized.
...
**Effect on ADR-000.** This ADR resolves ADR-000's Deferred Decision "Cross-language protocol compatibility," to the extent stated above — namely, what each language package must independently guarantee. It leaves runtime interoperability between language packages open as a distinct, separately deferred question; it does not resolve "Infrastructure adapter interfaces and conformance," a different Deferred Decision this ADR does not address. It amends no AAM membership list and supersedes no existing rule.
...
**Left deferred.** Whether and how independently conformant language packages can be made to compose or exchange events with one another at runtime is left to a future, dedicated ADR — this one deliberately does not address it; see **Scope**. What happens when two language packages are later found to implement the same ADR-required behavior inconsistently is likewise left open, pending that future ADR or its own decision. "Infrastructure adapter interfaces and conformance," the other Deferred Decision named in ADR-000 alongside cross-language protocol compatibility, remains unresolved and is not addressed here. The specific repository directory layout, build tooling, and CI/publishing mechanics for each language package are tooling decisions this ADR constrains (independent versioning, per-language OpenSpec) but does not itself make.
