...
- String-valued CloudEvents extension carrying an additional numeric-domain rule: `executionunits`, when non-null, under the rule below.
...
`depth` needs no new rule either, and is deliberately absent from every list above: ADR-001 already limits it to a non-negative integer, and an integer of any size has an exact, unambiguous decimal string form without narrowing its domain at all — unlike `executionunits`, whose original domain (any finite number, including non-integers) has no single canonical string form until a numeric representation is fixed. How `depth` is actually carried on a CloudEvent is an encoding choice for whichever ADR defines that transformation, not a domain question this ADR has any stake in.
