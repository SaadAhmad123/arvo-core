...
import { exceptionToSpan, getTelemetryContext, logToSpan } from '../OpenTelemetry';
...
import { ArvoEventData, CloudEventExtension, CreateArvoEvent, CreateArvoEventResult } from './types';
...
>(
  ...
  const activeContext = getTelemetryContext(telemetry?.context.traceparent || null);
  const activeTracer = telemetry?.tracer || trace.getTracer("ArvoEvent Creation")
