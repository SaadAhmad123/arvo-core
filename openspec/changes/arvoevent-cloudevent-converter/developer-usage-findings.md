...
**Closed.** `default.spec.ts`'s round-trip losslessness suite now has a dedicated "through the real wire" nested block: the fully populated, minimal, and `-0` executionunits cases each get a `JSON.stringify`/`JSON.parse` variant alongside the existing in-memory one, plus a double-round-trip idempotency check, plus an explicit regression guard asserting the documented non-UTC-`time` gap *stays* reproducing (so it can't silently start passing — either from an undocumented fix or from something newly swallowing a real mismatch — without the test noticing). 100% coverage of `src/cloudevent/**` held throughout.
