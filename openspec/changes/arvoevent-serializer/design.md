...
- **Reusing an existing plain-object type guard for the `specversion` check** instead of writing a new one — checked, nothing to reuse. `ArvoEvent/json.ts` has its own `isPlainObject`, but it takes `object`, not `unknown`, and isn't exported; this class's own `isRecord` needs to narrow from `unknown` (a freshly-`JSON.parse`d value) and stays local and unexported, small enough that duplicating its one line costs less than adding a shared export two call sites don't yet justify.
