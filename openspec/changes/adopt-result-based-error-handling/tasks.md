...
- [x] 3.1 **REVISED after 3.1–3.2 were first ticked.** `static parse<T, D>(param: unknown, options): ArvoEvent<T, D>` is now an unwrap around `tryParse` — `const result = ArvoEvent.tryParse<T, D>(param, options); if (result.ok) return result.value; throw result.error;` — not a direct delegate to `new ArvoEvent(...)`. Reworked for exact signature symmetry with `tryParse` (`unknown` in both, matching the convention's own point that `parse`/`tryParse` are one operation exposed two ways); `param`'s type changed from `ArvoEventParam<T, D>` to `unknown`, trading compile-time field checking on the common construction path for that symmetry. Still no logic of its own — see `design.md`'s "Reconsidered, and adopted."
