...
- [ ] 2.1 In `src/types.ts`, add `Result<R, E>` and `AsyncResult<R, E>` as thin aliases over `neverthrow`'s types — the only file that imports from `neverthrow` directly
- [ ] 2.2 Consumer-facing TSDoc on both aliases, per the Documentation in source convention — state the shape, no provenance
...
- [ ] 3.2 Add `static tryParse<T, D>(input, options): Result<ArvoEvent<T, D>, ArvoEventValidationError>` — calls `new ArvoEvent(input, options)` inside a try/catch, `Ok` on success, `Err` when the thrown error is `ArvoEventValidationError`, re-throws anything else
...
- [ ] 3.5 TSDoc for `parse`/`tryParse`, consumer-facing, describing the throw/`Result` relationship without restating the design rationale (that stays in `design.md`)
...
- [ ] 5.1 In `tests/ArvoEvent/index.spec.ts`, rewrite the `ArvoEvent.safeParse` describe block against `tryParse`: success returns `Ok` carrying a real `ArvoEvent` instance, failure returns `Err` carrying `ArvoEventValidationError` with populated `issues`, non-object top-level input fails cleanly
