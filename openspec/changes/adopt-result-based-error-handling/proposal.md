...
`ArvoEvent` is the first class this convention has ever been applied to, and the convention's exact shape for a class with a validating constructor — which of the three (constructor, `tryX`, `X`) holds the logic, which two are matched pairs, what a `Result`'s error channel is and isn't allowed to swallow — gets worked out for real here, not merely applied from an already-complete rule. `project.md` is updated in step as each piece is settled, so the general statement stays canonical there; this change is the proving ground, not a one-off.
...
- Adds `ArvoEvent.parse`, an unwrap around `tryParse` (not a direct delegate to `new ArvoEvent(...)`) — identical `(input: unknown, options?)` signature to `tryParse`, no logic beyond returning the success value or throwing the failure, so a consumer scanning static methods finds an exact matched pair without needing to know `new` already is the throwing entry point.
...
- Adds `Result<R, E>` and `AsyncResult<R, E>` (`= Promise<Result<R, E>>`) to `src/types.ts` — `arvo-core`'s own plain, structural types, not a re-export of any library's. Adopts `neverthrow` as an internal implementation dependency (reuse-check and alternatives in `design.md`), used to build a `Result` internally and converted to the plain public shape at each boundary by a shared adapter. `neverthrow` is never exported and never appears in a public type.
