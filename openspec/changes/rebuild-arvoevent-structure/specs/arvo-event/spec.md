...
The system SHALL accept any finite number as `executionunits`, placing no constraint on its sign or magnitude, and SHALL attach no interpretation to its value.
...
The system SHALL NOT validate `traceparent` or `tracestate`.
...
- **WHEN** `traceparent` or `tracestate` carries a value of any shape
- **THEN** construction succeeds
...
The system SHALL establish structural validity when an event is created. A structurally invalid event SHALL NOT come into existence.

An event already in hand SHALL NOT require rechecking.

#### Scenario: Invalid input never yields an event
- **WHEN** input violates any structural rule
- **THEN** creation fails
...
The system SHALL provide a means of establishing validity that reports its outcome rather than raising.

#### Scenario: Valid plain data
- **WHEN** plain data satisfying every structural rule is admitted
- **THEN** the outcome is successful
- **AND** the resulting event is indistinguishable from a directly created one

#### Scenario: Invalid plain data
- **WHEN** plain data violating a structural rule is admitted
- **THEN** the outcome reports failure without raising
- **AND** the failure carries the same explanation a creation failure would
