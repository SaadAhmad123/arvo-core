...
- [x] 11.1 Add `tests/ArvoEvent/errors.spec.ts` testing `errors.ts`'s own exported surface directly: `ArvoEventValidationIssue` construction, `describeValue`'s branches (string, number, boolean, bigint, function, symbol, array, plain object, cyclic/unserializable object) via `ArvoEventValidationError`'s rendered message, single vs. aggregated-issue message formatting, truncation of a long string and a long serialized object, and that `issues` on the thrown error is frozen
- [x] 11.2 Add `tests/ArvoEvent/json.spec.ts` testing `walkPayload` and `walkFlatMap` directly (not only indirectly through `ArvoEvent`): every rejected value class, cycle detection including mutual references and the legitimately-repeated-value case, undefined handling in map and array position, path reporting including non-identifier keys, deep freezing of the returned value, and the top-level shape guards for both functions
- [x] 11.3 Add `tests/ArvoEvent/validator.spec.ts` testing `validateArvoEvent` directly: the non-object top-level guard, unrecognised-key rejection, every field rule, both cross-field constraints including their permitted cases, `skipPayloadValidation` behaviour, and issue aggregation — independent of whatever `ArvoEvent`'s constructor happens to pass through
- [x] 11.4 Run `pnpm test:coverage` and drive coverage of `src/ArvoEvent/` (`errors.ts`, `json.ts`, `validator.ts`, `index.ts`, `opentelemetry.ts`, `types.ts` where executable) to 100% lines and branches, adding whatever targeted cases close the remaining gaps
- [x] 11.5 **100% statements, branches, functions, and lines** across `src/ArvoEvent/`, `src/types.ts`, and `src/utils.ts` — 244/244 statements, 169/169 branches, 29/29 functions, 227/227 lines, with an empty uncovered-lines column. 335 tests over six files.

  Measured with `pnpm vitest run --coverage tests/ArvoEvent tests/utils.spec.ts`, which excludes `tests/factory/` because its 18 tests still fail against the rebuilt envelope — the documented consequence of leaving `src/factory/` untouched (10.1), not a regression here. `src/factory/` is not instrumented in either run, so the figures above are unaffected by excluding it; `pnpm test:coverage` will report the same numbers once that suite is fixed.

  Two gaps found and closed, both invisible through `ArvoEvent`'s constructor: `opentelemetry.ts` had no direct test at all, and its `Span`-versus-`SpanContext` discrimination and `traceFlags` padding were never exercised — `tests/ArvoEvent/opentelemetry.spec.ts` added. `utils.ts:30` appeared uncovered only when the run was scoped to `tests/ArvoEvent`; `tests/utils.spec.ts` already covers both offset-sign branches.
